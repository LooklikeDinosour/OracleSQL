...
-- 주의할 점(그룹절에 사용할 컬럼만, SELECT절에 사용)
...
--그룹함수를 WHERE절에 적용할 수 없습니다. -> 다음에 배울거임 HAVING이라고
...
--------------------------------------------------------------------------------
--HAVING
--그룹의 조건은 HAVING 절을 사용합니다.
--월급 10000이상되는 JOB ID 찾기
SELECT JOB_ID, SUM(SALARY)
FROM EMPLOYEES
GROUP BY JOB_ID
HAVING AVG(SALARY) >= 10000;

--각 부서의 사원 수 구하고, 30명 이상 되는 부서 찾기
...
--부서아이디가 50번 이상이나 부서를 그룹화 시키고 그룹평균급여 5000이상만 출력
