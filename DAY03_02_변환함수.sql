...
--TO_DATE(문자, 날짜포맷) 문자와 날짜포맷 형태를 동일하게해주기.
...
SELECT FIRST_NAME, COMMISSION_PCT * 100 FROM EMPLOYEES; --NULL 연산시 => NULL
SELECT FIRST_NAME, NVL(COMMISSION_PCT, 0) * 100 FROM EMPLOYEES;

--NVL2(컬럼, NULL이 아닌경우처리, NULL인 경우처리)
SELECT NVL2(NULL, '널아니쥬','널이쥬') FROM DUAL;
SELECT FIRST_NAME,
       SALARY,
       COMMISSION_PCT,
...
--COALESCE(A, B) NVL과 유사 (NULL일 경우에 0으로 치환)
