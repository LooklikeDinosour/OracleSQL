...
SELECT EMPLOYEE_ID, LAST_NAME, FIRST_NAME, HIRE_DATE, SALARY FROM EMPLOYEES;
--2. 모든 사원의 이름과 성을 붙여 출력하세요. 열 별칭은 name으로 하세요.
SELECT LAST_NAME ||' '||FIRST_NAME AS NAME FROM EMPLOYEES; -- AS name 썼는데 출력은 대문자로 나옴
--3. 50번 부서 사원의 모든 정보를 출력하세요.
SELECT * FROM EMPLOYEES WHERE DEPARTMENT_ID IN 50; -- ID = 50;
--4. 50번 부서 사원의 이름, 부서번호, 직무아이디를 출력하세요.
SELECT FIRST_NAME, LAST_NAME, DEPARTMENT_ID, JOB_ID FROM EMPLOYEES WHERE DEPARTMENT_ID IN 50; -- ID = 50;
...
SELECT FIRST_NAME, LAST_NAME, SALARY FROM EMPLOYEES WHERE DEPARTMENT_ID IN 60 ORDER BY SALARY DESC;
--12. 직무아이디가 IT_PROG 이거나, SA_MAN인 사원의 이름과 직무아이디를 출력하세요.
SELECT FIRST_NAME, LAST_NAME, JOB_ID FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'SA_MAN'; -- JOB_ID IN('IT_PROG', 'SA_MAN')
--13. Steven King 사원의 정보를 “Steven King 사원의 급여는 24000달러 입니다” 형식으로 출력하세요.
SELECT FIRST_NAME ||' '||LAST_NAME ||' 사원의 급여는 '||SALARY||'달러 입니다.'
FROM EMPLOYEES WHERE FIRST_NAME IN 'Steven' AND LAST_NAME IN 'King';
--14. 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 출력하세요. (컬럼:job_id)
SELECT FIRST_NAME, LAST_NAME, JOB_ID FROM EMPLOYEES WHERE JOB_ID LIKE '___MAN'; ---%MAN
--15. 매니저(MAN) 직무에 해당하는 사원의 이름과 직무아이디를 직무아이디 순서대로 출력하세요.
SELECT FIRST_NAME, LAST_NAME, JOB_ID FROM EMPLOYEES WHERE JOB_ID LIKE '___MAN' ORDER BY JOB_ID ASC;
