...
SELECT FIRST_NAME, LTRIM(FIRST_NAME, 'A') FROM EMPLOYEES; -- 왼쪽에서 처음 발견되는 문제 A제거
...
--중첩
SELECT REPLACE('HELLO WORLD', 'HELLO', 'BYE') FROM DUAL; --이거를 아래에 쏙 붙여넣기 
SELECT REPLACE(REPLACE('HELLO WOLRD ~!', 'HELLO', 'BYE'),' ', '') AS RESULT FROM DUAL; -- 안에 있는 것 부터 실행됨
...
SELECT CONCAT(FIRST_NAME, LAST_NAME), REPLACE(HIRE_DATE, '/' , '') AS HIRE_DATE FROM EMPLOYEES  ORDER BY FIRST_NAME ASC;
...
SELECT '(02)'||SUBSTR(PHONE_NUMBER, INSTR(PHONE_NUMBER, '.'), LENGTH(PHONE_NUMBER)) AS RESULT FROM EMPLOYEES;
